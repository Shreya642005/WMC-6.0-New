  ...
  useEffect(() => {
    ...
    missions.forEach((mission) => {
        ...
        html: `
          <div class="relative">
            <div class="w-8 h-8 rounded-full border-2 border-white shadow-lg flex items-center justify-center animate-pulse" 
                 style="background-color: ${urgencyColors[mission.urgency]}">
              <svg class="w-4 h-4 text-white" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 2L13.09 8.26L20 9L13.09 9.74L12 16L10.91 9.74L4 9L10.91 8.26L12 2Z" />
              </svg>
            </div>
            ${mission.urgency === "critical" ? '<div class="absolute inset-0 w-8 h-8 rounded-full border-2 border-red-400 animate-ping"></div>' : ""}
          </div>
        `,
      ...
      const marker = leaflet
        .marker(mission.coordinates, { icon: customIcon })
        .addTo(map)
        .bindPopup(`
          <div class="bg-gray-900 text-white p-3 rounded-lg min-w-64">
            <h3 class="font-bold text-lg mb-2 text-red-400">${mission.title}</h3>
            ...
            <div class="mt-2 flex gap-2">
              <span class="px-2 py-1 rounded text-xs font-medium" style="background-color: ${urgencyColors[mission.urgency]}20; color: ${urgencyColors[mission.urgency]}">
